...
export default function Home() {
  ...
  useEffect(() => {
    axios.get('https://innovart-backend.onrender.com/api/products?destacados=1')
    ...
    axios.get('https://innovart-backend.onrender.com/api/users?destacados=true')
      .then(res => setArtesanosDestacados(res.data.slice(0, 8)))
      .catch(() => setArtesanosDestacados([]));
    axios.get('https://innovart-backend.onrender.com/api/resenas?destacadas=1')
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans bg-gradient-to-br from-white via-sky-100 to-blue-200">
      ...
      <nav className="w-full bg-blue-900 shadow-lg flex items-center justify-between px-10 py-4 z-30 relative sticky top-0 left-0">
        {/* Logo y menú a la izquierda */}
        <div className="flex items-center gap-12">
          <img src="/logo_innovart_white.png" alt="Logo InnovArt" className="h-16 w-auto drop-shadow-lg transition-transform hover:scale-105" />
        ...
        {/* Búsqueda rápida y PERFIL a la derecha */}
      ...
      {/* Pie de página */}
      <footer className="w-full bg-blue-900 text-white text-sm py-3 px-6 flex justify-between items-center z-10 relative">
